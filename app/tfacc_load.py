...
OL = Overlay('fpga-data/design_1.bit', download=True)
#if not OL.is_loaded():
#  OL.download()

# get clock frequency from hardware configuration file
with open('fpga-data/design_1.hwh', 'r') as f:
  for line in f:
...
print('*** Load "srmon.mot" to sr_cpu memory')
import numpy as np
...
#  if i % 16 == 0: print("\n%04x : "%i, end='')
  wd = buf[i]<<24 | buf[i+1]<<16 | buf[i+2]<<8 | buf[i+3]
#  print("%08x "%wd, end='')
  mmio.write(i, int(wd))
